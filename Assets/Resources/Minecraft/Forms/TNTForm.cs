...
[CreateAssetMenu(menuName = "Forms/TNT Form")]
public class TNTForm : BaseForm
{
    ...
    public override void FormAction(float context)
    {
        ...
        {
            var pos = info.point;
            bullet.GetComponent<TNT_Block_Bullet>().SetTargetPosition(pos);
        }
        else {
            var dir = PlayerController.Instance._playerCamera.forward;
            bullet.GetComponent<TNT_Block_Bullet>().SetDirection(dir);
        }
    }
}
